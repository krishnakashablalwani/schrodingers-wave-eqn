...
def ej3(nx: int, ny: int, nz: int, m: float, L: float) -> float:
    return ((nx * math.pi) ** 2 + (ny * math.pi) ** 2 + (nz * math.pi) ** 2) * (sp.constants.hbar**2) / (2.0 * m * (L**2))

def psi3(nx: int, ny: int, nz: int, X: np.ndarray, Y: np.ndarray, Z: np.ndarray, L: float) -> np.ndarray:
    return (2.0 / L) ** 1.5 * np.sin(nx * math.pi * X / L) * np.sin(ny * math.pi * Y / L) * np.sin(nz * math.pi * Z / L)
...
def plot_psi_levels(triplets: list[tuple[int, int, int]], m: float, L: float, out_path: Path) -> None:
    x = np.linspace(0, L, 1000)
    y0 = L * 0.5; z0 = L * 0.5
    E = np.array([ej3(int(nx), int(ny), int(nz), m, L) / sp.constants.e for (nx, ny, nz) in triplets])
    uniq = np.unique(np.sort(E)); spc = float(np.min(np.diff(uniq))) if len(uniq) > 1 else 1.0
    s = 0.35 * spc
    plt.figure(figsize=(6, 8))
    for (nx, ny, nz), En in zip(triplets, E):
        psi_slice = (2.0 / L) ** 1.5 * np.sin(nx * math.pi * x / L) * np.sin(ny * math.pi * y0 / L) * np.sin(nz * math.pi * z0 / L)
        y = En + s * psi_slice
...
def plot_density_levels(triplets: list[tuple[int, int, int]], m: float, L: float, out_path: Path) -> None:
    x = np.linspace(0, L, 1000)
    y0 = L * 0.5; z0 = L * 0.5
    E = np.array([ej3(int(nx), int(ny), int(nz), m, L) / sp.constants.e for (nx, ny, nz) in triplets])
    uniq = np.unique(np.sort(E)); spc = float(np.min(np.diff(uniq))) if len(uniq) > 1 else 1.0
    plt.figure(figsize=(6, 8))
    for (nx, ny, nz), En in zip(triplets, E):
        r = ((2.0 / L) ** 1.5 * np.sin(nx * math.pi * x / L) * np.sin(ny * math.pi * y0 / L) * np.sin(nz * math.pi * z0 / L)) ** 2
...
def main() -> None:
    ...
    modes = [(nx, ny, nz) for nx in range(1, NMAX + 1) for ny in range(1, NMAX + 1) for nz in range(1, NMAX + 1)]
    ...
    E_sorted = np.array([ej3(nx, ny, nz, m, L) / sp.constants.e for (nx, ny, nz) in modes_sorted])
    plt.figure(figsize=(6, 4))
    plt.plot(np.arange(1, K + 1), E_sorted, marker="o")
    plt.xlabel("mode index k"); plt.ylabel("Energy (eV)"); plt.title("Energy vs mode index (3D)"); plt.tight_layout(); plt.savefig(pics / "energy_vs_n.png", dpi=200); plt.close()
    ...
    for i, (nx, ny, nz) in enumerate(sel, start=1):
        ...
        plt.xlabel("x (m)"); plt.ylabel("y (m)"); plt.title(f"Eigenfunction ψ z=L/2 ({nx},{ny},{nz})")
    ...
    X2, Y2 = X, Y; Z2 = np.full_like(X2, z0)
    plt.figure(figsize=(9, 3.6))
    for idx, t in enumerate(times, start=1):
        p = np.zeros_like(X2, dtype=np.complex128)
        for cc, (nx, ny, nz) in zip(c, N_VALS):
            p += cc * psi3(int(nx), int(ny), int(nz), X2, Y2, Z2, L) * np.exp(-1j * ej3(int(nx), int(ny), int(nz), m, L) * t / sp.constants.hbar)
        plt.subplot(1, 3, idx)
    ...
    print("Particle in a 3D infinite well"); print(f"L={L:.2e} m, m={m:.4e} kg"); print(f"E(1,1,1)={E111:.3f} eV, E(2,1,1)={E211:.3f} eV")
